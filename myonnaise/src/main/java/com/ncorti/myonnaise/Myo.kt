...
class Myo(private val device: BluetoothDevice) : BluetoothGattCallback() {
    ...
    private var serviceImu: BluetoothGattService? = null //add
    ...
    private var characteristicImu: BluetoothGattCharacteristic? = null//add
    ...
    override fun onServicesDiscovered(gatt: BluetoothGatt, status: Int) {
        ...
        serviceEmg?.apply {
            ...
            val emgCharacteristics = listOf(characteristicEmg0, characteristicEmg1, characteristicEmg2, characteristicEmg3)
        ...
        serviceImu?.apply {
            ...
            val imuCharacteristic = characteristicImu
    ...
    override fun onCharacteristicChanged(gatt: BluetoothGatt, characteristic: BluetoothGattCharacteristic) {
        ...
        if (characteristic.uuid.toString().endsWith(CHAR_IMU_POSTFIX)) {
            val imuData = characteristic.value
            byteReader.byteData = imuData

            // We receive 10 bytes of IMU data.
            dataProcessor.onNext(byteReader.getBytes(IMU_ARRAY_SIZE))
            dataProcessor.onNext(byteReader.getBytes(IMU_ARRAY_SIZE))
        }
